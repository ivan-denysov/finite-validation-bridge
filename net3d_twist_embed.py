...
# D2 DECISIVE: twist transmission WITH dynamic embedding (KT=0.006).
# Clamps as before (x<0.08 axis z; 0.46<x<0.54 axis y): lock variables held AND
# clamp-slab nodes frozen. Interior: full axiom + node motion under traffic tension.
# Persistence length ~9.5 links = gap scale -> transmission now possible in principle.
...
for t in range(1, TICKS+1):
    ...
    if t in (2000, 5000, 8000):
        print(f"\n=== t={t}  pers={np.mean(pers[-300:]):.3f}  [{time.time()-t0:.0f}s] ===")
        profile(P, 1.0 - c)
np.save('/home/claude/twist_embed_c.npy', c)
np.save('/home/claude/twist_embed_P.npy', P)
