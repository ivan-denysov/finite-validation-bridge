...
def run(arc_frac, tag, seed):
    ...
    wnode = wn_c.copy(); wdir = wd_c.copy()
    W = len(wnode)
    ...
    for t in range(1, TICKS+1):
        ...
        if t in (1000, 2000, 4000, 6000, 8000):
            ...
            print(f"[{tag}] t={t}: straight={s_ch:.3f} (no-flow {max(nofl,0.03):.3f}, bg {s_bg:.3f})  "
                  f"u_ch={u_ch:5.2f}  in tube={intube.sum()}/{W}  circ={circ:+.3f}  [{time.time()-t0:.0f}s]")
